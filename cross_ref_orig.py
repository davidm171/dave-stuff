...
def crossRefs(lang, build_path):
	...
	def relPath(file):
		...
		return ellipsis
	# ---------------------------------------------------------------------------------------------------------------------------------
	# Function to create a dictionary of file address versus h1 pairs -----------------------------------------------------------------
	def htmHeading(htmList):
		htmHeading = {}
		for file in htmList:
			text1 = openFile(file)
			text = text1.replace('\n', '').replace('\r', '')				# Remove carriage returns and line feeds
			rawHeading = re.findall(r"<h1.+?</h1>",text)					# Remove all other tags
			if rawHeading != []:
				raw = rawHeading[0]
				Heading = re.sub(r"<.*?>", "", raw)
				htmHeading [file] = Heading
			else:
				htmHeading [file] = "null"
		return htmHeading
	# ---------------------------------------------------------------------------------------------------------------------------------	
	...
	def guideToDir(ref, fileHeadingDict, file):
		guideName = ref[0]
		...
		if guideName in guideToDirMap:
			...
			if len(ref) == 1:
				...
				linkInfo = firstPageCrossRef(guideDirectory, section, fileHeadingDict, linkInfo, file)
															
			elif len(ref) == 2:
				section = ref[1]
				print "SECTION is: ", section
				linkInfo = constructCrossRef(guideDirectory, section, fileHeadingDict, linkInfo, file)
	...
	def firstPageCrossRef(guideDirectory, section, fileHeadingDict, linkInfo, file):
		# print "IT GETS INTO SUB, linkInfo at start is: ", linkInfo
		# print fileHeadingDict
		# x = raw_input("STOP!!!")
		for fileAddress, headingText in fileHeadingDict.iteritems():
			if (str(guideDirectory).lower() in str(fileAddress).lower()) and (str(section).lower() in str(fileAddress).lower()):
				# print "+++The section is: ", section
				# print "+++The fileAddress is: ",  fileAddress
				# print "+++The headingText is: ", headingText
				htmRef = re.findall(r'Subsystems\\(.+?\.htm)',fileAddress)
				relativeRef = str(htmRef[0])
				# href = "..\..\..\\" + relativeRef
				href = relPath(file) + "\\" + relativeRef				#Why need the to add \\?????
	...
	# ---------------------------------------------------------------------------------------------------------------------------------	
	# Search for cross ref guide directory address in all address, and section, and construct cross ref -------------------------------
	def constructCrossRef(guideDirectory, section, fileHeadingDict, linkInfo, file):
		print "IT GETS INTO SUB, linkInfo at start is: ", linkInfo
		# print fileHeadingDict
		# x = raw_input("STOP!!!")
		for fileAddress, headingText in fileHeadingDict.iteritems():
			if (str(guideDirectory).lower() in str(fileAddress).lower()) and (section == headingText):
				# print "+++The section is: ", section
				# print "+++The fileAddress is: ",  fileAddress
				# print "+++The headingText is: ", headingText
				htmRef = re.findall(r'Subsystems\\(.+?\.htm)',fileAddress)
	...
	for file in htmList:
		...
		findItalics = re.findall(r"<i>.+?</i>", fileText)
		print "\nThe file is: ", file
		if "system_parameters" not in str(file):  						# Don't process the system parameters directory
			if findItalics:
				for hit in findItalics:
					print "The italics are: ", hit
					ref  = processItalics(findItalics)
					print "The reference is ", ref
					refInfo = guideToDir(ref, fileHeadingDict, file)
